...
parser.add_argument('--alph'  , type=str, default='1.0,10.0,0.0,1.0,1.0')  # origin 1, 100, 5
...
parser.add_argument("--drop_freq"   , type=int  , default=1000, help="how often to decrease learning rate")
...
parser.add_argument('--logrho0_freq', type=int, default=100)
...
def compute_loss(net, x, rho_x, nt):
    ...
    Jc, cs, weights, position = OTFlowProblem_ex(x, rho_x, net, [0, 1], nt=nt, stepper="rk4", alph=net.alph, is_1d=False,  alphaa=args.alphaa)
...
if __name__ == '__main__':
    ...
    print(args.data)
    x0, rho_x0, d_net = toy_data.inf_train_gen(args.data, batch_size=args.batch_size,require_density=True)
    x0 = cvt(torch.from_numpy(x0))
    rho_x0 = cvt(torch.from_numpy(rho_x0))

    x0val, rho_x0val, d_net2 = toy_data.inf_train_gen(args.data, batch_size=args.val_batch_size,require_density=True)
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.viz_freq == 0:
            nSamples = 20000
            p_samples = toy_data.inf_train_gen(args.data, batch_size=nSamples, require_density=False)
