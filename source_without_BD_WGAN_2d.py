...
def OTFlowProblem_ex(x, Phi, tspan, nt, stepper="rk4", alph=[1.0, 1.0], is_1d=False, alphaa=1):
    """
    ...
    # TT = torch.exp(-h/alphaa*TT)/z.shape[0]


    costL = torch.sum(z[:, -2])  # dv, 0.5*rho*v^2+0.5*alpha*rho*g^2
    # costR = torch.sum(z[:, -3])    # \Phi penalty
    costV = torch.sum(z[:, -3])   # v penalty

    # cs = [costL, costR, costV]
    cs=[costL,costV]

    # return dot(cs, alph)  , cs
    return sum(i[0] * i[1] for i in zip(cs, alph)), cs, z[:, -1], z[:, 0:2]
...
def integrate_ex(x, net, tspan, nt, stepper="rk4", alph=[1.0, 1.0], intermediates=False, alphaa=50):
    """
    ...
    z = pad(z, (0, 1, 0, 0), value=1.0/x.shape[0])
