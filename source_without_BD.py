...
def OTFlowProblem_1d(x, Phi, tspan, nt, stepper="rk4", alph=[1.0, 1.0, 1.0], alphaa=1,jjj=0, device=device):
    ...
    new_weight = torch.exp(z[:, -4] + z[:, -2]).unsqueeze(-1)*w_data
    ...
    if jjj == 0:
        ...
        costC = 0.5 * d * log(2 * pi) - torch.mean(l) + 0.5 * torch.mean(
            torch.sum(torch.pow(z[:, 0:d], 2), 1, keepdims=True)) + torch.mean(z[:, -2])+z[0,-4]
...
def integrate_ex(x, net, tspan, nt, stepper="rk4", alph=[1.0, 1.0, 1.0], intermediates=False, alphaa=10):
    """
