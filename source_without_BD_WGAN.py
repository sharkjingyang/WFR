...
def OTFlowProblem_ex(x, rho_x, Phi, tspan, nt, stepper="rk4", alph=[1.0, 1.0, 1.0], is_1d=False, alphaa=1):
    """
    ...
    costV = torch.sum(z[:, -4])   # v penalty

    cs = [costL, costR, costV]

    # return dot(cs, alph)  , cs
    return sum(i[0] * i[1] for i in zip(cs, alph)), cs, z[:, -1], z[:, 0:1]
...
def integrate_ex(x, net, tspan, nt, stepper="rk4", alph=[1.0, 1.0, 1.0,], intermediates=False, alphaa=50):
    """
