...
parser.add_argument('--alph'  , type=str, default='1.0,100.0,1.0')
...
if __name__ == '__main__':
    ...
    x0, _ = toy_data.inf_train_gen(args.data, batch_size=args.batch_size, require_density=False)
    x0 = cvt(torch.from_numpy(x0))

    x0val, _ = toy_data.inf_train_gen(args.data, batch_size=args.val_batch_size, require_density=False)
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.val_freq == 0 or itr == args.niters:
            with torch.no_grad():
                ...
                genModel = integrate(y1, net, [1, 0.0], nt_val, stepper="rk4", alph=net.alph)


                mmd1 = MMD(x0val.unsqueeze(-1), genModel[:, 0].unsqueeze(-1))
        ...
        if itr % args.viz_freq == 0:
            with torch.no_grad():
                ...
                plot1d_OT(net, p_samples, y, nt_val, sPath, doPaths=True, sTitle='{:s}  -  loss {:.2f}  ,  C {:.2f}  ,  alph {:.1f} {:.1f}  '
        ...
        if itr % args.sample_freq == 0:
            ...
            x0, _ = toy_data.inf_train_gen(args.data, batch_size=args.batch_size, require_density=False)  # load data batch
