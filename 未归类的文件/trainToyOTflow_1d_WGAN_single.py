...
def compute_loss(net, x, rho_x, nt, t0=0, t1=1):
    # Jc , cs = OTFlowProblem_BD(x, net, [0,1], nt=nt, stepper="rk4", alph=net.alph, is_1d=True,alphaa=args.alphaa)
    Jc, cs, weights, position = OTFlowProblem_ex(x, rho_x, net, [t0, t1], nt=nt, stepper="rk4", alph=net.alph, is_1d=True, alphaa=args.alphaa)
...
if __name__ == '__main__':
    ...
    # optimD = torch.optim.RMSprop(netD.parameters(), lr=args.lrD)
    optimD = torch.optim.Adam(netD.parameters(), lr=args.lrD, betas=[0.0, 0.9])
    # optim2 = torch.optim.Adam(netD.parameters(), lr=args.lrD, betas=(args.beta1, 0.999))
    ...
    for itr in range(1, args.niters + 1):
        ...
        while i < Diters:
            ...
            with torch.no_grad():
                loss, costs, weights, position = compute_loss(net, x0, rho_x0, nt=args.nt, t0=0, t1=1)
        ...
        if itr % args.val_freq == 0 or itr == args.niters:
            ...
            with torch.no_grad():
                ...
                test_loss1 = -torch.dot(fx1, test_weights)
                test_loss = test_loss+test_loss1*args.lambda1
                # add to print message
                log_message += '    {:9.3e}  {:9.3e}  {:9.3e}  {:9.3e} {:9.3e} '.format(
                    test_loss, test_costs[0], test_costs[1], test_costs[2], test_loss1
                )
