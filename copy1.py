...
parser.add_argument('--alph'  , type=str, default='1.0, 10') # origin 1, 100, 5
...
parser.add_argument('--lrG'          , type=float, default=0.005) # origin without GAN: 0.1
...
def compute_loss(net, x, nt, t0=0, t1=1, alphaa=10):
    # Jc , cs = OTFlowProblem_BD(x, net, [0,1], nt=nt, stepper="rk4", alph=net.alph, is_1d=True,alphaa=args.alphaa)
    Jc, cs, weights, position = OTFlowProblem_ex(x, net, [t0, t1], nt=nt, stepper="rk4", alph=net.alph, is_1d=False, alphaa=alphaa)
...
if __name__ == '__main__':
    ...
    optimG = torch.optim.Adam(net.parameters(), lr=args.lrG, weight_decay=args.weight_decay)
    ...
    log_msg = (
        '{:5s}  {:6s}   {:9s}  {:9s}  {:9s}  {:9s}     {:9s}  {:9s}   {:9s} {:9s}'.format(
            'iter', ' time','lossG', 'L (L_2)', 'V(velocity)','W1', 'valLoss', 'valL',  'valV', 'valW1'
        )
    )
    ...
    for itr in range(1, args.niters + 1):
        ...
        while i < Diters:
            ...
            with torch.no_grad():
                loss, costs, weights, position = compute_loss(net, x0, nt=args.nt, t0=0, t1=1, alphaa=args.alphaa)
                # loss_1, costs_1, weights_1, position_1 = compute_loss(net, y.squeeze(), nt=args.nt, t0=1, t1=0,
                #                                                       alphaa=1000)

            fx = netD(position).squeeze()
            fy = netD(y).squeeze()
            gp = compute_gradient_penalty(netD, real_samples=y, fake_samples=position, d=2, device=device)
            ...
            # fy2 = netDD(x0).squeeze()
            # gp2 = compute_gradient_penalty(netDD, real_samples=x0, fake_samples=position_1, d=2, device=device)
        ...
        x0 = cvt(torch.from_numpy(x0))
        y = cvt(torch.randn(x0.shape[0], d))
        loss, costs, weights, position = compute_loss(net, x0, nt=args.nt, alphaa=args.alphaa)
        # loss_1, costs_1, weights_1, position_1 = compute_loss(net, y.squeeze(), nt=args.nt, t0=1, t1=0,
        #                                                       alphaa=1000)
        ...
        # print("loss:{}, loss1:{}".format(loss.item(), loss1.item()))
        # if (loss>loss1.item()*0.01)or(itr<10):
        #     lossG = loss1
        # else:
        #     lossG = loss + loss1 * args.lambda1
        lossG = loss + loss1 * args.lambda1
        # lossG = loss1
        optimG.zero_grad()
        lossG.backward()
        ...
        log_message = (
            '{:05d}  {:6.3f}   {:9.3e}  {:9.3e}  {:9.3e}  {:9.3e}'.format(
                itr, time_meter.val , lossG, costs[0], costs[1], loss1
            )
        )

        # validate
        if itr % args.val_freq == 0 or itr == args.niters:
            x0val = toy_data.inf_train_gen(args.data, batch_size=args.val_batch_size, require_density=False)
            x0val = cvt(torch.from_numpy(x0val))
            y = cvt(torch.randn(x0.shape[0], d))
            with torch.no_grad():
                net.eval()
                netD.eval()
                test_loss1, test_costs, test_weights, test_position = compute_loss(net, x0val, nt=nt_val, alphaa=args.alphaa)
                # test_loss2, test_costs2, test_weights2, test_position2 = compute_loss(net, y.squeeze(),
                #                                                                       nt=nt_val,
                #                                                                       alphaa=1000)
                ...
                test_loss = test_loss1+test_loss_1*args.lambda1
                # add to print message
                log_message += '    {:9.3e}  {:9.3e}  {:9.3e}  {:9.3e} '.format(
                    test_loss, test_costs[0], test_costs[1], test_loss_1
                )
                ...
                if test_loss.item() < best_loss:
                    ...
                    }, os.path.join(args.save, start_time + '_{:}_alph{:}_{:}_m{:}_checkpt.pth'.format(args.data,int(alph[0]),int(alph[1]),m)))
                    net.train()
            ...
            with torch.no_grad():
                ...
                plot2d(net, p_samples, y, nt_val, sPath, doPaths=True,
                          sTitle='{:s}  -  loss {:.2f}  ,  C {:.2f}  ,  alph {:.1f} {:.1f}  '
                                 ' nt {:d}   m {:d}  nTh {:d}  '.format(args.data, best_loss, best_costs[1], alph[0],
                                                                        alph[1], nt, m, nTh))
        ...
        if itr % args.sample_freq == 0:
            ...
            x0 = toy_data.inf_train_gen(args.data, batch_size=args.batch_size,require_density=False)  # load data batch
            x0 = cvt(torch.from_numpy(x0))
