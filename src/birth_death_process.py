# add source term and apply birth-death process to approximate the weights

# act as OTFlowProblem.py, include compute loss(a kind of new loss based on birth death process)
...
def OTFlowProblem_BD(x, Phi, tspan, nt, stepper="rk4", alph=[1.0, 1.0, 1.0], is_1d=False, alphaa=0.1):
    """
    ...
    if is_1d:
        z = pad(x.unsqueeze(-1), (0, 3), value=0)
    else:
    ...
    if stepper == 'rk4':
        for k in range(nt):
            z0, V = stepRK4(odefun, Phi, z, Phi, alph, tk, tk + h,alphaa=alphaa)
            tk += h
            z = birth_death(z0, V, alphaa=alphaa,device=device)
    elif stepper == 'rk1':
        for k in range(nt):
    ...
    costC = torch.mean(C(z,alphaa=alphaa))
...
def stepRK4(odefun, net, z, Phi, alph, t0, t1, alphaa):
    """
    ...
    K = h * odefun(z0, t0, Phi, alph=alph, alphaa=alphaa)
    z = z0 + (1.0 / 6.0) * K

    K = h * odefun(z0 + 0.5 * K, t0 + (h / 2), Phi, alph=alph,alphaa=alphaa)
    z += (2.0 / 6.0) * K

    K = h * odefun(z0 + 0.5 * K, t0 + (h / 2), Phi, alph=alph,alphaa=alphaa)
    z += (2.0 / 6.0) * K

    K = h * odefun(z0 + K, t0 + h, Phi, alph=alph,alphaa=alphaa)
    z += (1.0 / 6.0) * K

    z1 = pad(z[:, :z.shape[1]-3], (0, 1, 0, 0), value=t0+h)
    Vx = net(z1)
    Vx = Vx-torch.mean(Vx.squeeze())
...
def integrate_BD(x, net, tspan, nt, stepper="rk4", alph=[1.0, 1.0, 1.0], intermediates=False, alphaa=0.1):
    """
    ...
    if intermediates:  # save the intermediate values as well
        ...
        if stepper == 'rk4':
            for k in range(nt):
                z0, V = stepRK4(odefun, net, zFull[:, :, k], net, alph, tk, tk + h,alphaa=alphaa)
                tk += h
                zFull[:,:,k+1]= birth_death(z0,V,alphaa=alphaa,device=device)
        elif stepper == 'rk1':
            for k in range(nt):
                zFull[:, :, k + 1] = stepRK1(odefun, net, zFull[:, :, k], net, alph, tk, tk + h,alphaa=alphaa)
                tk += h
        ...
        if stepper == 'rk4':
            for k in range(nt):
                z0,V = stepRK4(odefun,net, z, net, alph, tk, tk + h,alphaa=alphaa)
                tk += h
                z = birth_death(z0,V,alphaa=alphaa,device=device)
        elif stepper == 'rk1':
            for k in range(nt):
                z = stepRK1(odefun, net,z, net, alph, tk, tk + h,alphaa=alphaa)
                tk += h
...
def C(z,alphaa=0.1):
    ...
    return -(torch.sum(-0.5 * math.log(2 * math.pi) - alphaa*torch.pow(z[:, 0:d], 2) / 2, 1, keepdims=True) + l.unsqueeze(1))


def odefun(x, t, net, alph=[1.0, 1.0, 1.0], alphaa=0.1):
    """
    ...
    dv = 0.5 * alphaa * torch.sum(torch.pow(dx, 2), 1, keepdims=True)
...
def birth_death(z, V, alphaa=0.1, device=device):
    """
    z: tensor  nex - by - d + 4, inputs
    V: nex-by-1, rate
    """
    idx = torch.rand(V.shape)
    z1 = z[(V>=0).squeeze()]
    V1 = V[V>=0]
    z1 = z1[(alphaa*V1 < torch.rand(V1.shape).to(device)).squeeze()]

    z2 = z[(V<0).squeeze()]
    V2 = V[V<0]
    z3 = z2[(alphaa*torch.abs(V2) > torch.rand(V2.shape).to(device)).squeeze()]

    new_z = torch.cat((z1,z2,z3), dim=0)
